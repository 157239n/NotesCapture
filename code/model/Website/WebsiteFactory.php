...
class WebsiteFactory {
    ...
    public function get(int $websiteId): Website {
        ...
        return new Website($this->mysqli, $this->highlightFactory, $websiteId, $row["website_url"], $row["category_id"], $row["title"], $user_handle);
    }
    ...
    public function all(): array {
        ...
        $user_handle = $this->session->getCheck("user_handle");
        $websites = [];
        if (!$answer = $this->mysqli->query("select website_url, website_id, category_id, title from websites where user_handle = '$user_handle'")) throw new WebsiteNotFound();
        while ($row = $answer->fetch_assoc()) $websites[] = new Website($this->mysqli, $this->highlightFactory, $row["website_id"], $row["website_url"], (int)$row["category_id"], $row["title"], $user_handle);
        return $websites;
    }
    ...
    public function allUnderStrictCategory(Category $category): array {
        ...
        while ($row = $answer->fetch_assoc()) $websites[] = new Website($this->mysqli, $this->highlightFactory, $row["website_id"], $row["website_url"], $categoryId, $row["title"], $row["user_handle"]);
        return $websites;
    }
    ...
    public function new(Category $category, string $url, string $title = ""): Website {
        ...
        if ($title == "") $title = $url;
        if (!$this->mysqli->query("insert into websites (user_handle, website_url, category_id, title) values ('$user_handle', '" . $this->mysqli->escape_string($url) . "', $categoryId, '" . $this->mysqli->escape_string($title) . "')")) Logs::error($this->mysqli->error);
