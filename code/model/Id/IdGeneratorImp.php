...
class IdGeneratorImp implements IdGenerator {
    ...
    private function newId(string $field, string $table) {
        ...
        $hasId = false;
        if ($answer = $this->mysqli->query("select $field from $table where $field = $id"))
            if ($row = $answer->fetch_assoc()) $hasId = true;
